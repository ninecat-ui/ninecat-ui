<template>
  <ul class="tree-ul">
    <li class="tree-li">
      <svg t="1585220115926" class="n-tree-icon point" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="2222" width="10" height="10" :style="{
      ...
      <input v-model="isChecked" v-if="root.showCheckbox" @change="handlerChecked" type="checkbox" class="tree-checkbox point" />
      <div class="tree-content point" @click="handlerClick">
        <Render></Render>
      </div>
    </li>
    <div v-if="data[root.defaultProps.children].length && data.expand">
      <node v-for="item of data[root.defaultProps.children]" :key="item[root.nodeKey]" :data="item"></node>
    </div>
  </ul>
</template>
